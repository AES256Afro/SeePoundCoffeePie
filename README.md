...
Every school also follows one no-assumed-knowledge rule. A lesson defines each required term, explains why the idea matters, restores the needed context, identifies the exact starting input, gives one action at a time, shows the expected result and acceptable variation, and explains how to stop, undo, or recover. This is a shared teaching standard, not a separate course, learner category, diagnosis field, or simplified track.

The expansion includes an optional Reality versus Fiction path across programming, Linux, cybersecurity, networking, and AI. Each unit separates a familiar claim from what is plausible, what is missing, how the real system works, how to inspect it safely, and what the evidence can and cannot prove. The Local Models and LLMs school also teaches model types, model files, tokens and context, precision, quantization, common quant names, hardware fit, and local-versus-hosted data and operating boundaries from the beginning.
