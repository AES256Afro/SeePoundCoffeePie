...
const privateMarkers = [
  ...
  ...unpublishedCppJavaScriptMarkers.map((marker) => marker.value),
]
...
if (
  ...
  throw new Error('The production bundle check requires Practical C++ to remain unpublished.')
}
...
const unpublishedCppTeachingDataAssets = emittedAssets.filter(({ name, contents }) => (
  name.endsWith('.json')
  && (
    name.startsWith('cpp-collections-records-course-packed')
    || contents.includes(firstUnpublishedCppLessonId)
  )
))
if (unpublishedCppTeachingDataAssets.length > 0) {
  throw new Error(
    `Unpublished Practical C++ teaching data appeared in `
    + `${unpublishedCppTeachingDataAssets.map(({ name }) => name).join(', ')}.`,
  )
}
...
console.log(
  ...
  + `${initialJavaScriptAssets.length} initial JavaScript assets, and one lazy foundation teaching asset.`,
)
