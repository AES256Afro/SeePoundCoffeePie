...
describe('beginner lesson interactions', () => {
  ...
  it('uses plain teaching labels in the editable lesson workspace', async () => {
    ...
    expect(screen.getByText('Words and code explained')).toBeTruthy()
  ...
  it('opens the first editable lesson with visible context and restores only the editor', async () => {
    ...
    fireEvent.click(within(guide).getByText('Words and code explained'))
  ...
  it('keeps every Other subjects course open in a compact, non-repeating card', () => {
    ...
    for (const course of academyCourses) {
      ...
      expect(cardQueries.getAllByText(course.outcome)).toHaveLength(1)
      if (course.summary !== course.outcome) expect(cardQueries.queryByText(course.summary)).toBeNull()
