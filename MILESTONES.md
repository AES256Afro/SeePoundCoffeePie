...
- **M065, Practical C# project two:** Build a small records tool using lists, methods, and explicit input validation.
...
- **M350, core academy-scale release:** Declare the first academy expansion stable only after shared architecture, open access, optional refreshers, cross-platform labs, Linux, networking, cybersecurity, local models, credentials, maintenance ownership, accessibility, privacy, security, staging, production, and recovery have durable evidence. Later universal teaching-clarity and truth-in-technology phases extend this baseline without weakening it.

## Phase 24: no-assumed-knowledge teaching standard

These milestones apply to every school and every learner. They do not create a separate course, learner category, or profile mode. The author must identify and explain the knowledge hidden inside phrases such as `simply`, `just`, `obviously`, `as usual`, or `you already know`. A learner never has to disclose a diagnosis or learning profile to receive clear structure, context, recovery, and accessibility. The product does not infer, request, verify, or store a health, disability, or diagnosis label from learning choices or behavior.

- **M351, no-assumed-knowledge contract:** Publish the [No-assumed-knowledge standard](docs/curriculum/NO_ASSUMED_KNOWLEDGE_STANDARD.md) as a product, authoring, accessibility, and review contract. Complete when every lesson identifies its assumptions, defines required terms, supplies context without lowering the intended outcome, and creates no course, profile, event, or learner-record field for a diagnosis or inferred learning category.
- **M352, visible scope before commitment:** Show the goal, purpose, new words, activity type, number of required actions, time range, browser or learner-machine boundary, required resources, state that may change, saved stopping points, and optional depth before a learner starts a unit or lab. Time is planning information, not a countdown or performance measure.
- **M353, predictable unit anatomy:** Use the stable learner-visible order in the no-assumed-knowledge standard: location and saved state; goal; plan and scope; starting point; optional context; concrete example; notice or prediction; explanation; one task; expected result; feedback; recap; save point and recovery; then Next, Stop, and optional depth. Complete when a learner does not have to rediscover how each page works and required context is never hidden in a disclosure.
- **M354, one clear next action:** Give each normal learning state one visually dominant verb-and-object action and place alternatives nearby without making them compete. Complete when the primary action remains obvious at narrow widths, 200 percent zoom, and keyboard focus, and focus moves to the result or another purposeful location after activation.
- **M355, meaningful chunking:** Break long explanations into sections that each answer one question. Use progressive disclosure for extra depth, raw diagnostics, alternate examples, and reference detail, while keeping required teaching, assessed facts, safety boundaries, stop conditions, and recovery visible without opening a disclosure.
- **M356, context that does not disappear:** Keep school, path, course, module, unit, current step, completion state, and the reason for the current exercise visible or one action away. Back, Forward, refresh, bookmarks, and return links must preserve the same context.
- **M357, stop and resume without reconstruction:** Save the exact unit section, exercise step, lab checklist item, permitted editor draft, disclosure state, and learner-confirmed lab checkpoint allowed by the privacy contract. Return with a plain recap of what was finished, what remains, the next small action, and anything the site cannot know about a learner-controlled machine. Never guess that a local command, service, cleanup step, or recovery action succeeded.
- **M358, planned pauses without pressure:** Place optional short pauses after concentrated sections and durable stopping points around 35 to 50 minutes of work. Never use countdowns, overdue labels, fake loss-of-progress or lost-reward warnings, or streak punishment to force continuation. Keep real data-loss, safety, cost, and unsafe-stop warnings visible when they apply.
- **M359, focus and sensory controls:** Add reversible Default, Reading focus, and Practice focus views; reduced motion; restrained color; no flashing or decorative particles; optional code-line focus; and the ability to hide nonessential navigation without hiding location, safety, context, primary action, recovery, or exit controls. Remember the selected view privately without creating a learner label or a separate progress route.
- **M360, readable scaling hierarchy:** Separate page, course, module, lesson, body, label, control, code, and help scales. Pass at 200 percent text resize and a 320 CSS pixel equivalent viewport, with increased browser text size, narrow screens, light and dark preferences where supported, no control or focused content covered by fixed regions, and no two-direction page scroll. A necessary wide code, table, or diagram region may scroll inside a labeled container with a linear equivalent.
- **M361, literal language and vocabulary recovery:** Prefer direct sentences, define terms before abbreviations, label figurative examples as examples, and let a learner open and close a keyboard-accessible local glossary definition without losing reading position or focus. Humor and themes may support memory but may not carry an instruction.
- **M362, example before abstraction:** Introduce a concrete, inspectable example before a general rule, diagram, formula, or library. Provide a second example with changed surface details so the learner can see what remains the same.
- **M363, calm error recovery:** Treat errors as tool information. State what happened, what work and progress stayed safe, where to look, one safe next check, and how to try again. Preserve the learner's work and keep the exact raw message available under a named technical-details control without presenting it as the only explanation or repeatedly announcing it while the learner types.
- **M364, retrieval and repetition with control:** Revisit ideas after increasing intervals using recognition, recall, prediction, ordering, repair, explanation, and transfer. Let learners pause or shorten a review session and explain why an item returned without ranking or shaming them. Base scheduling on documented learning evidence and learner choices, never an inferred diagnosis or learner category.
- **M365, assessment access without outcome dilution:** Offer equivalent prepared-evidence, keyboard, accessible editor, and learner-controlled lab routes when they demonstrate the same published objective. State format, time range, item or stage count, breaks, resumption, evidence, retakes, feedback, and appeal before an attempt. Preserve the skill and scoring standard, require no voice or video, and store no health, disability, or diagnosis label.
- **M366, beginner-clarity release gate:** Enforce the standard's authoring, layout, keyboard, screen-reader, reduced-motion, sensory, resumption, assessment, and learner-review gates across representative routes. Include 200 percent text resize, 320 CSS pixel reflow, deliberate errors, interruption and resume, focus views, and real beginner review. Record what was directly tested, reject health or disability fields, and keep unsupported universal-access claims out of release notes.

## Phase 25: reality versus fiction across the academy

Reality checks are teaching units, not entertainment recaps. They compare a popular claim, dramatic scene, marketing statement, or common myth with observable systems and dated evidence. Cybersecurity examples remain conceptual and defensive, use prepared synthetic evidence, and do not include an active intrusion or exploitation route. Authoring may begin earlier, but public Phase 25 units depend on the generic academy, source and freshness contracts, and M351 through M366.

- **M367, reality-check editorial contract:** Adopt the [Reality versus Fiction curriculum](docs/curriculum/REALITY_VS_FICTION_CURRICULUM.md). Every comparison includes `The claim or scene`, `What is plausible`, `What is exaggerated or missing`, `The real underlying concept`, `Safe exercise or observation`, `Defensive or professional takeaway`, and `Short knowledge check`, plus visible claim evidence, reality evidence, evidence limits, and review dates.
- **M368, evidence labels and uncertainty:** Implement the registry labels `documented`, `supported-inference`, `illustrative`, `disputed`, and `outdated`. Visible source records separately identify direct observation, primary-source statement, measured result, reconstruction, and unknown. Complete when a learner can distinguish what is known, inferred, illustrative, disputed, outdated, or still unknown without mistaking confidence for proof.
- **M369, time-compression visual:** Show the real sequence, people, permissions, dependencies, waiting, failed attempts, and recovery that fiction compresses into one screen or command. Keep the visual usable as text and without sensational countdowns.
- **M370, programming myths from zero:** Cover the RVF-100 core comparisons: a complete program from one burst of typing, correct behavior on the first run, one language controlling every layer, error messages as meaningless noise, generated code as finished software, and more code as proof of quality. Contrast them with requirements, layers and permissions, iteration, error evidence, testing, review, maintenance, and teamwork.
- **M371, programming scenes and real execution:** Trace source, parser or compiler, dependencies, runtime, input, output, errors, tests, build artifacts, permissions, and deployment for prepared fictional scenes. Never require the learner to accept unexplained code as magic.
- **M372, Linux myths and realities:** Cover the RVF-200 core comparisons: a terminal as a hacker screen, routine root or `sudo`, automatic security, zero total cost, an arbitrary directory tree, one universally best distribution, universal Windows-to-Linux migration claims, and absolute gaming-compatibility claims. Use dated, workload-specific evidence and link optional deeper Linux study without making it a gate.
- **M373, dramatic command versus safe administration:** Reconstruct a fictional one-line repair as discovery, scope, ordinary-user versus administrative authority, backup, documentation, change, verification, monitoring, rollback, and ownership. The core exercise uses prepared L0 evidence and rewards a safe stop when evidence is insufficient. Any optional L1 observation is a separate platform-specific lab with a prepared-evidence alternative.
- **M374, instant-hack timeline:** Break a fictional intrusion into access conditions, hidden dependencies, privileges, detection chances, people and roles, evidence, defensive response, and recovery. Use prepared synthetic evidence only, expose authorization and uncertainty, and provide no active intrusion, exploitation, movement, or public-target instructions.
- **M375, password, encryption, and single-control fiction:** Explain credential reuse, phishing, rate limits, multi-factor authentication, recovery, password storage, key material, encryption boundaries, isolated-system limits, endpoint controls, and why antivirus or another single product does not prove complete safety. Provide no credential-testing or real-target procedure.
- **M376, malware, logs, and remote-control fiction:** Compare self-spreading code, persistence, command channels, privilege, user interaction, software flaws, endpoint and network controls, and the claim that deleting one log removes all evidence. Use prepared benign traces to teach distributed evidence, containment, recovery, and logging limits without live harmful payloads.
- **M377, attribution, forensics, retaliation, and teamwork fiction:** Show why an IP address, username, file string, accent, clock, or code fragment does not prove an actor; why infrastructure may belong to another victim; why counterattack can exceed authority or cause harm; and why safe response requires coordinated roles. Teach evidence integrity, alternatives, confidence, responsible reporting, authorized containment, and recovery.
- **M378, network fiction:** Cover the RVF-400 core comparisons: signal bars versus end-to-end service, an IP address versus a person, VPN privacy limits, incognito boundaries, bandwidth versus latency and throughput, what ping proves, NAT versus firewall policy, and BGP policy versus geographic shortest path. Use prepared diagrams, documentation addresses, and synthetic records without scanning or contacting a public target.
- **M379, AI fiction and marketing claims:** Cover the RVF-500 core comparisons: human-like understanding, larger-is-always-better claims, automatic local privacy, fine-tuning as current factual storage, retrieval as a truth guarantee, temperature as a truth control, one benchmark as universal proof, production training in minutes, and cost-free quantization. Keep the core L0, prepared, and no-compute; identify system and data boundaries; and link deeper model study without requiring it.
- **M380, comparison interaction:** Build one accessible bookmarkable unit with the `Before we compare` context block and all seven canonical sections from M367. Keep the short knowledge check and answer-specific feedback directly open. Show claim sources, reality sources, evidence label, version or platform scope, `lastVerifiedAt`, and `reviewDueAt` near the comparison. Use a side-by-side evidence area only when space permits and preserve a plain linear reading order at narrow widths and 200 percent text resize.
- **M381, reality-transfer self-check:** Give a new claim in an open A0 exercise and ask the learner to separate claim, observation, assumption, evidence, mechanism, risk, uncertainty, and safe verification. Accept a well-supported `not enough information` conclusion and provide immediate answer-specific feedback and another attempt. Completion records practice only and do not claim professional competence.
- **M382, reality-check publication gate:** For the complete Phase 25 path, verify five courses, 20 modules, and 39 comparison units against the registry rather than copied page totals. Require all seven sections, no-assumed-knowledge context, separate claim and reality sources, evidence labels, dated and versioned scope, `lastVerifiedAt`, `reviewDueAt`, rights review, originality, open routes, L0 core exercises, prepared alternatives, safety, accessibility, expert review where risk warrants it, and no operational attack guidance. An overdue or withdrawn comparison keeps a visible dated canonical route rather than becoming locked or hidden.

## Phase 26: local models versus hosted and corporate models

This phase expands the Local Models and LLMs school with a complete comparison path. It does not declare local or hosted systems universally better. It gives beginners enough context to choose deliberately and enough evidence to revise that choice later. Every published unit remains directly open, offers `Start now` with context in place, and has an L0 prepared, no-compute route. Public teaching depends on M351 through M366 and the applicable M109, M110, M121 through M132, and M296 through M330 contracts it actually uses. M406 additionally depends on the applicable M133 through M140 identity, assessment, evidence, privacy, and credential decisions; those credential dependencies do not delay open teaching.

- **M383, what a model is:** Explain a model as learned numerical behavior used to transform an input into an output. Separate model, application, ordinary code, database, search index, prompt, tool, memory store, and user interface with one inspectable system map.
- **M384, model families and modalities:** Teach classification, regression, ranking, generation, embedding, reranking, language, code, image, audio, vision-language, and multimodal models. Complete when learners choose a family by task rather than calling every AI system an LLM.
- **M385, model roles:** Compare base, pretrained, instruction-tuned, chat, reasoning-oriented, embedding, reranker, safety, speech, vision, and task-specific models. State that labels are not standardized guarantees and must be checked against the model card and evaluation.
- **M386, weights, parameters, checkpoints, and artifacts:** Show how architecture, learned parameters, weight files, tokenizer, configuration, prompt template, adapters, licenses, model cards, and checksums fit together. A model filename alone is not accepted as full provenance.
- **M387, tokens, context, memory, and retrieval:** Explain tokenization, context windows, generated tokens, system and user input, cached attention state, external retrieval, conversation storage, and why a model does not automatically remember previous sessions.
- **M388, deployment-shape map:** Compare consumer chat, provider API, managed enterprise service, organization-controlled cloud deployment, private hosted service, local desktop application, local command-line runtime, and isolated offline runtime. Show who controls each layer.
- **M389, local-versus-hosted data-flow lab:** Use an L0 prepared-evidence route to map input, output, files, retrieval sources, logs, telemetry, identity, network hops, retention, deletion, support access, and backups for one local and one hosted design. Use only synthetic data and never infer privacy from the word `local` or `enterprise` alone.
- **M390, provider-claim verification:** Teach learners to inspect current official terms, product tier, training-use statement, retention, regional processing, abuse monitoring, stateful features, subprocessors, deletion, encryption, and contractual scope. Date every example because provider behavior changes.
- **M391, privacy and security tradeoffs:** Compare physical access, malware, untrusted model files, package supply chains, local network exposure, cloud identity, API keys, tenant controls, provider access, logging, updates, incident response, and recovery.
- **M392, cost and capacity tradeoffs:** Compare hardware purchase, electricity, storage, time, maintenance, idle capacity, per-token or subscription pricing, staff, data movement, support, scaling, and shutdown. Separate a short experiment from sustained operation.
- **M393, quality and capability tradeoffs:** Compare task quality, model size, specialization, tool use, context, multimodality, safety controls, latency, update cadence, reproducibility, and evaluation access using the same task and scoring rubric.
- **M394, reliability and control tradeoffs:** Compare offline operation, Internet dependency, service limits, queues, outages, version changes, model retirement, local hardware failure, backups, rollback, observability, and who owns each repair.
- **M395, open source, open weights, and proprietary:** Explain that visible or downloadable weights do not automatically make the training data, training code, license, or use rights open. Require separate checks for code, weights, data, documentation, trademark, acceptable use, and redistribution.
- **M396, precision without mystery:** Explain FP32, FP16, BF16, FP8 context, INT8, and INT4 as ways numbers can be represented, then show why training precision, stored weight precision, compute precision, and cache precision may differ.
- **M397, quantization and quants:** Explain quantization as representing model values with fewer bits or a different scheme to reduce memory and sometimes improve speed, with possible quality and compatibility tradeoffs. A `quant` is a particular quantized build, not a new trained personality.
- **M398, quant-name reading:** Teach common GGUF names such as `Q4_0`, `Q4_K_S`, `Q4_K_M`, `Q5_K_M`, `Q6_K`, and `Q8_0` as runtime-specific recipes. Explain the broad bit-level signal, mixed-tensor suffixes, importance-matrix context, and why no suffix is a universal quality promise.
- **M399, model-fit estimator:** Build a transparent estimator from parameter count, stored bits per weight, file size, runtime overhead, context, cache, batch, concurrency, CPU memory, accelerator memory, and offload. Show the calculation, uncertainty range, and measured correction rather than only `will run` or `will not run`.
- **M400, formats and runtimes:** Compare framework checkpoints, `safetensors`, GGUF, adapters, tokenizer files, runtime conversion, llama.cpp, Ollama, MLX, and other reviewed lanes by purpose and compatibility. Do not imply that renaming a file converts it.
- **M401, first local model lab:** Provide an L0 prepared, no-compute route plus reviewed Windows, WSL, macOS, and Linux lanes for a learner-controlled L2 lab. Review source, license, exact revision, files, size, checksum, runtime, bind address, telemetry, resource estimate, expected behavior, stop conditions, cleanup, and offline verification before running a small model. The site does not download, run, or receive the model or private prompts.
- **M402, fair local-versus-hosted comparison lab:** Use one harmless synthetic task set, equivalent instructions, recorded model and service versions, repeated trials, quality rubric, latency, resource or price record, privacy boundary, failure notes, and reproducibility limits. Require explicit consent before any optional provider request, send no private content, state cost and retention scope, and do not crown a winner without stated requirements.
- **M403, choose the simplest sufficient system:** Compare ordinary code, search, rules, a small task model, a local generative model, a hosted API, retrieval, and fine-tuning. Complete when the learner can reject an LLM where it adds cost or uncertainty without benefit.
- **M404, migration and portability:** Teach exportable prompts, test sets, data, embeddings, adapters, interfaces, logs, and configuration; provider-specific features; version pinning; replacement tests; and an exit plan without claiming every model is interchangeable.
- **M405, local-and-hosted design capstone:** Choose a fictional personal, school, or small-organization use case, compare at least three deployment shapes, map data, evaluate quality, estimate cost and hardware, threat-model both sides, choose or reject a system, and document a fallback.
- **M406, model-comparison credential:** Pilot an applied credential only after the learner explains model types, reads a quant name, calculates a fit range, maps local and hosted data flows, evaluates equivalent outputs, and defends a decision with evidence. Credential requirements control issuance only; all related teaching, lab guidance, skills outlines, and rubrics remain directly open, and no LLM makes the final grading decision.
- **M407, model-comparison release gate:** Verify plain-language beginner review, L0 prepared no-compute access, open routes, optional context without access gates, Windows, WSL, macOS, and Linux claims, exact tool and source versions, provider-claim freshness, local lab preflight and cleanup, no browser-side model processing, no voice requirement, the no-assumed-knowledge standard, assessment integrity, accessibility, and honest limits.

## Phase 27: expanded academy evidence

- **M408, selected overview-image production:** Combine the selected stacked-course-pages and honest-product-composite directions into one simple versioned social image. Keep the four language symbols, open courses, lesson and practice context, generous whitespace, no slogan, and no decorative technology. Verify 1200 by 630 output, meaningful alt text, preview metadata, cache-safe versioning, and live retrieval.
- **M409, cross-cutting curriculum index:** Make beginner context, reality checks, model comparison, optional refreshers, schools, paths, labs, and credentials discoverable through distinct URL-addressable groups without turning the catalog into one undifferentiated list. Keep every published item directly open and label credentials as evidence records rather than permission to learn.
- **M410, expanded academy release:** Declare these extensions stable only after registry-generated course, module, and unit counts; authored content; direct open routes; optional context; evidence; accessibility; safety; privacy; source freshness; staging; production; and rollback, public withdrawal, or retirement plans pass for the same reviewed release. Reject any course, profile, analytics event, or learner-record field that stores or infers a diagnosis or learning category.

## Immediate working order

The 2026-08-31 source checkpoint implements the first representative open-academy vertical slice without declaring the wider academy complete. It adds a generic manifest outside the programming-language unions, fail-closed ownership and route validation, two directly open paths, two complete courses, three module pages, eight complete reading units, four optional preparation pages, prepared exercises, answer-specific retry feedback, source records, progress compatibility, lazy assets, and 19 checked-in canonical sitemap URLs. `What a model is` defines model boundaries before later model families, quantization, local execution, or hosted-service comparisons. `Programming on screen and at work` publishes the first two Reality versus Fiction comparisons. The slice stores no diagnosis or inferred learner category, creates no special course for a learning profile, and applies the no-assumed-knowledge contract to every learner. This is representative implementation evidence for parts of M101, M103 through M105, M109, and M112 through M115. It does not complete M102 while legacy programming locks remain, M106 through M108, M110 through M111, M116 through M120, any full school, manual beginner review, staging, or production.
...
The active release order remains the controlled M009 staging deployment and regression proof, the recorded manual beginner and accessibility review, and then M010 production. Both release milestones require more than a passing local build or hosted CI job. The owner authorized the first M101 vertical slice to proceed in source while those deployment gates remain separate. M101 through M120 establish the shared open academy. M121 through M132 and M141 through M160 establish lab safety, platform routes, model boundaries, and optional preparation before later school foundation slices. M133 through M140 proceed separately after D02 and D03 and do not delay open teaching.

M351 through M366 are horizontal requirements, not a separate learner track and not a reason to wait for every school to be complete. Apply them to each new academy slice before that slice is published. Phase 25 authoring follows the M367 contract and may proceed while shared components are built, but a public reality-check unit must pass the applicable M351 through M382 gates. Phase 26 teaching can proceed from the applicable local-model foundations and shared contracts without waiting for every M296 through M330 specialization; M406 remains a separate credential gate. M408 can proceed as a versioned asset task, M409 follows the stable content hierarchy, and M410 is the common release gate for the complete extension.
