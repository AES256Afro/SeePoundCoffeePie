...
The latest documented production baseline is the 2026-08-26 Phase 5B compatibility release. It has five courses, 30 modules, 150 lessons, four guided projects, and 100 editable runner assignments. The production site also has the OAuth flow, synchronized learner record, four runner toolchains, adaptive Practice, Code Reference, portfolio exports, and bookmarkable routes. Practical C++ is fully authored in private local source, but it remains unpublished.

As of 2026-08-28, M001 through M008 meet their local acceptance rules in the release-candidate source. M005 and M006 are complete only as unpublished candidate integrations. They have not been released. The evidence recorded below is local and does not claim hosted CI, staging, or deployment. M009 and M010 still require an exact reviewed commit, hosted CI, manual accessibility and beginner checks, staging evidence, and production authorization.
...
- **M002, fail-closed course release state:** Add a private release-state catalog whose unknown and unpublished entries always resolve to unavailable. Keep Practical C++ outside public course types, definitions, loaders, routes, sitemap, browser content, and runner assignments. Complete when tests prove every public lookup still fails closed.
...
- **M005, controlled Practical C++ integration:** Add the course outline, prerequisite display, exact lesson routes, Practice ownership, and Code Reference ownership behind the unpublished release state. Complete when local tests can exercise both states but the production build still chooses unpublished.
- **M006, protected assignment integration:** Register all 12 editable Practical C++ assignments only through the protected publication boundary. Add abort support so a learner leaving a page cannot leave a stale 45-second poll running. Complete when visible and private cases pass and unpublished grants still return not found.
...
- **M008, browser release gate:** Add Playwright coverage for catalog, prerequisites, direct lesson URLs, keyboard editing, mobile navigation, Practice, Code Reference, and unpublished boundaries. Include an automated accessibility pass, but keep manual review separate.
...
The completed local gates are M001 through M008. M005 and M006 remain behind the fail-closed unpublished state even though both candidate integrations now pass locally. The active order is an exact release-candidate commit and hosted CI, then M009 and M010. Staging begins only from that reviewed, CI-verified commit, and both release milestones require more than a passing local build.

Current M005 evidence exercises the complete candidate state through the same controlled publication decision used by the production build. The candidate has six catalog cards, Home and Learner Record continuation, the real 6-module and 30-lesson Practical C++ outline, exact prerequisites, canonical same-page lesson navigation, cached lazy content, Practice aggregation, all 11 Code Reference entries, and a 43-URL candidate sitemap. Its deterministic teaching JSON measures 42.222 kB raw and 14.300 kB gzip. The complete candidate application measures 747.504 kB raw and 217.952 kB gzip total JavaScript, 83.13 kB raw and 16.01 kB gzip total CSS, and 876.28 kB raw and 249.32 kB gzip aggregate transfer. The Practical Python route shared by both continuing courses measures 10.306 kB raw and 3.617 kB gzip. All unchanged caps pass, but total JavaScript has only 48 gzip bytes of headroom and must be rechecked after every source change. Eleven candidate Chromium cases verify prerequisites, first, middle, and final lesson history, one cached JSON request, title and focus, Home, Learner Record, portfolio ownership, Practice, Code Reference, and a truthful malformed-data failure. The production build still selects five courses, rejects both guessed Practical C++ routes, and emits no candidate JSON. This is local candidate evidence, not publication evidence.

Current M006 hardening binds every existing public runner grant and queued result to a deterministic SHA-256 revision of grading-only assignment data plus a checked revision of the evaluators, analyzers, runner images, coordinator, and Sandbox package. The browser receives only a keyed opaque binding. The Worker and coordinator recheck the underlying revision before accepting source, creating a sandbox, and returning stored output. Revoked, changed, or legacy work fails closed without exposing submitted source or stored output. Browser navigation, reset, exit, continue, and unmount cancel local grant, submit, response-reading, and polling work. The controlled Worker source validates all 12 Practical C++ assignments and the 112-assignment candidate registry through a pinned Wrangler dry run. The default Worker remains at 100 assignments and rejects all 12 private IDs. The local M006 acceptance rule is complete. Runner-image execution and a controlled staging window remain M009 evidence, not local proof.

Current M007 evidence from a clean local bundle check is:
...
- emitted JSON teaching data: 0 kB in the unpublished production build;
- aggregate transfer: 822.87 kB raw with 9.67 percent headroom, and 231.49 kB gzip with 8.32 percent headroom.

The initial-load headroom requirement is satisfied. The readable foundation curriculum compiles to one deterministic lazy packed asset instead of a large curriculum JavaScript module. The hidden Practical C++ teaching data compiles to one deterministic lazy JSON asset measuring 42.222 kB raw and 14.300 kB gzip. The isolated publication pieces measure 63.694 kB raw and 18.611 kB gzip JavaScript plus 105.916 kB raw and 32.911 kB gzip aggregate transfer. More importantly, the complete candidate-enabled application passes every unchanged category and aggregate limit at the M005 measurements above. Practical C++ remains unpublished and the production build emits no candidate JSON.

The complete current local release gate passes 78 Vitest files and 808 tests, 11 of 11 candidate Chromium cases, and 15 of 15 production Chromium cases. It also verifies deterministic foundation and Practical C++ generation, the complete candidate browser and Worker selections, all category and aggregate bundle limits, the fail-closed production bundle, pinned runner publication, and both local Wrangler dry runs with Docker disabled. The repository text-style check scans 299 tracked and release-candidate files. Hosted CI, manual keyboard and screen-reader review, 200 percent zoom, reduced-motion review, other browsers, runner-image execution, staging, and production remain outstanding release evidence. See the [Phase 5B browser gate record](docs/PHASE_5B_BROWSER_GATE.md).
