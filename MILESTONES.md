...
The current source-controlled publication candidate selects the complete Practical C++ course. It has six courses, 36 modules, 180 lessons, four guided projects, and 112 editable runner assignments. Practical C++ contributes 12 runner-backed lessons and 18 teaching-only lessons that must never receive execution grants. This source state is not live deployment evidence. M009 and M010 remain incomplete until the exact reviewed commit passes hosted CI, staging, manual accessibility and beginner review, and production verification.
...
M001 through M008 are implemented in the publication-candidate source. The source now selects the complete six-course application and exact 112-assignment runner registry. The active order is a clean reviewed commit and hosted CI, then M009 and M010. Staging begins only from that CI-verified commit, and both release milestones require more than a passing local build.
...
The dated local gate recorded 78 Vitest files and 808 tests, 11 of 11 candidate Chromium cases, and 15 of 15 five-course Chromium cases. The browser coverage is now being carried by the ordinary production-dist harness rather than a second candidate server. The full current release gate, exact commit, hosted CI, manual keyboard and screen-reader review, 200 percent zoom, reduced-motion review, other browsers, runner-image execution, staging, and production must all be recorded separately. See the [Phase 5B browser gate record](docs/PHASE_5B_BROWSER_GATE.md).
